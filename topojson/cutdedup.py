...
class _Cutdedup:
    ...
    def main(self, data):
        """
        Deduplication of linestrings that contain duplicates and 
        Cut the linestrings given the junctions of shared paths.

        The cut & dedup function is the third step in the topology computation.
        ...
        * start of iteration between cut.py and dedup.py        
        ...
        # --- 2 ---
        # then split each feature given the intersections 
...
def _cutdeduper(data):
    data = copy.deepcopy(data)
    Cutdedup = _Cutdedup()
    cd = Cutdedup.main(data)
    return cd
